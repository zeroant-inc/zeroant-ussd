    ...
    async run<T = unknown>(event: string, data: Record<any, any>): Promise<T> {
        ...
        for (const action of actions) {
            ...
            if (eventHandler.isValid()) {
                ...
                let result: T = await actionHandler.run<Promise<T>>(eventHandler.event)
...
export interface Event<T = any> {
    ...
    con(arg: Record<string, any>): EventPayload<T>;
    end(arg: string, ...args: string[]): string[];
    end(arg: Record<string, any>): EventPayload<T>;
}
export class Event {
...
export class EventText extends String {
    private _type: "CON" | "END" | undefined;
    constructor(value: any) {
        super(value);
        const type = this.substr(0, 3).trim();
        this._type = type as any;
    }
    get type(): "CON" | "END" {
        return this._type as any; // Replace with your actual implementation
    }
    get isCon() {
...
export class EventPayload<T = unknown> {
    constructor(private _type: "CON" | "END", private _payload: T, private _contentType?: string) {
    }
    get type(): "CON" | "END" {
        return this._type as any;
    }
